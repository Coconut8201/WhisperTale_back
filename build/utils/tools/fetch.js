...
exports.setVoiceModel = exports.getVoices = exports.fetchImage = exports.whisperCall = void 0;
const dotenv_1 = __importDefault(require("dotenv"));
dotenv_1.default.config();
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const findLatestFile = (directory, prefix) => {
    try {
        const allFiles = fs_1.default.readdirSync(directory);
...
const getVoices = (Saved_storyID, storyTale, voiceModelName) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    // 獲取排序後的第一個檔案
    const sortedFiles = fs_1.default.readdirSync(referPathDir).sort();
    const firstFile = sortedFiles.length > 0 ? sortedFiles[0] : null;
    if (!firstFile) {
        throw new Error(`no file found in ${referPathDir} `);
    }
    const promptText = yield (0, exports.whisperCall)(referPathDir, firstFile);
    if (!promptText) {
        return {
            audioFileName: '',
    ...
    const requestBody = {
        ref_audio_path: path_1.default.join(referPathDir, firstFile),
    ...
    try {
        ...
        return {
            audioFileName: '',
            audioBuffer: new ArrayBuffer(0),
            error: `語音生成失敗：${error.message}`
        };
    }
});
exports.getVoices = getVoices;
// 設定語音模型
const setVoiceModel = (modelName) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    console.log(`已找到以下兩個模型：${latestGptFile}, ${path_1.default.join(sovitsWeightsDir, latestSovitsFile)}`);
    const payload = {
        "gpt_model_path": latestGptFile,
        "sovits_model_path": path_1.default.join(sovitsWeightsDir, latestSovitsFile)
    };
    const requestOptions = {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    try {
        const response = yield fetch(`${process.env.GPT_SOVITS_VOICE_API}/set_model`, requestOptions);
...
exports.setVoiceModel = setVoiceModel;
