 ...
 */
const sdModelOption = (MODEL_NAME) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        let payload = {
            "sd_model_checkpoint": ""
        };
        ...
        const updateResponse = yield fetch(`${process.env.stable_diffusion_api}/sdapi/v1/options`, updateOptions);
...
exports.sdModelOption = sdModelOption;
const getSDModelList = () => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const response = yield fetch(`${process.env.stable_diffusion_api}/sdapi/v1/sd-models`);
        if (!response.ok) {
...
const GenImg_prompt_En = (story_slice) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const prompt = `StableDiffusion is a deep learning-based text-based graph model that supports generating 
   new images by using prompt words to describe elements to include or omit. I introduce the concept of Prompt in 
   the StableDiffusion algorithm, also known as prompts. The following prompts are used to guide the AI ​​painting model 
   to create images. They contain various details of the image, such as the appearance of the characters, the background, 
   the color and lighting effects, and the theme and style of the image. The format of these prompts often contains weighted
   numbers in brackets to specify the importance or emphasis of certain details. For example, "(masterpiece:1.5)" indicates 
   that the quality of the work is very important, and multiple brackets have similar effects. In addition, if brackets are used,
   such as "{blue hair:white hair:0.3}", this means that blue hair and white hair are merged, with blue hair accounting for 0.3. 
   The following are examples of using prompts to help the AI ​​model generate images: masterpiece, (best quality), highly detailed, 
   ultra-detailed, cold, solo, (1girl), detailedeyes, shinegoldeneyes) (longliverhair) expressionless, (long sleeves, puffy sleeves), 
   (white wings), shinehalo, (heavymetal: 1.2), (metaljewelry), cross-lacedfootwear (chain), (Whitedoves: 1.2) The prompts you can choose 
   include: Color light dark pale deep Weather time golden hour lighting strong rim light intense shadows in the rain rainy days sunset 
   cloudy Buildings in the baroque architecture in the romanesque architecture streets (Roman streets) in the palace (Palace) at the 
   castle(The exterior of the castle is the background) in the castle(The interior of the castle is the background) in the street 
   (On the street) in the cyberpunk city (In the cyberpunk city) rainy night in a cyberpunk city with glowing neon lights (At the lighthouse 
   (In a misty onsen (Hot springs) by the moon (In a bar, in bars (Bars) in a tavern (Izakaya (Japanese arch (Torii) in a locker room 
...
exports.GenImg_prompt_En = GenImg_prompt_En;
let generated_imageprompt_array;
/**
 * 生成圖片prompt(輸出應為英文，這樣才不會出錯)
 */
const GenImg_prompt_En_array = (story_array) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        for (let i = 0; i < story_array.length; i++) {
            ...
            console.log(`第 ${i} 次生成`);
            generated_imageprompt_array.push(yield (0, exports.GenImg_prompt_En)(story_slice));
        }
...
exports.GenImg_prompt_En_array = GenImg_prompt_En_array;
